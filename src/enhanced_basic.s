...
*	OPT	CRE			* EASy68k directive - not needed for vasm
	ORG	$00401000		* past the vectors in a real system

* Entry point - Jump directly to BASIC
	XDEF	START_ENTRY		* Export symbol for linker
START_ENTRY
...
* PCC2 (PCCchip2) Registers for MVME167
PCC2CHIP    EQU $FFF42000
PCCSCCTICR  EQU PCC2CHIP+$1E    * SCC Transmit Interrupt Control Register
PCCSCCRICR  EQU PCC2CHIP+$1F    * SCC Receive Interrupt Control Register
PCCTPIACKR  EQU PCC2CHIP+$25    * Transmit Pseudo IACK Register
PCCRPIACKR  EQU PCC2CHIP+$27    * Receive Pseudo IACK Register

* CD2401 Serial Controller for MVME167
* Register definitions from Linux serial167 driver
* These offsets are already correct for MVME167 byte ordering
...
CyCMR       EQU $02        * Channel Mode Register
CyTCOR      EQU $26        * TX Clock Option Register
CyTBPR      EQU $28        * TX Baud Rate Period Register
CyRCOR      EQU $27        * RX Clock Option Register
CyRBPR      EQU $29        * RX Baud Rate Period Register
CyCOR1      EQU $08        * Channel Option Register 1
CyCOR2      EQU $09        * Channel Option Register 2
CyCOR3      EQU $0A        * Channel Option Register 3
CyCOR4      EQU $0B        * Channel Option Register 4
CyCOR5      EQU $0C        * Channel Option Register 5
CyCOR6      EQU $0D        * Channel Option Register 6
CyCOR7      EQU $0E        * Channel Option Register 7
CyMSVR1     EQU $DE        * Modem Signal Value Register 1
CyMSVR2     EQU $DF        * Modem Signal Value Register 2
...
CySCHR1     EQU $1F        * Special Character Register 1
CySCHR2     EQU $20        * Special Character Register 2
CySCHR4     EQU $1C        * Special Character Register 4
CySTCR      EQU $0C        * Special Transmit Command Register

* CD2401 Channel Command Register commands
CyINIT_CHAN EQU $10        * Initialize channel
CyENB_XMTR  EQU $08        * Enable transmitter
CyENB_RCVR  EQU $02        * Enable receiver

* CD2401 additional registers for PIACK method
CyLICR      EQU $26        * Local Interrupting Channel Register
CyTEOIR     EQU $85        * Transmit End of Interrupt Register
CyREOIR     EQU $84        * Receive End of Interrupt Register

* Interrupt and command constants
CyTxMpty    EQU $02        * Transmit Buffer Empty interrupt
CyRxData    EQU $08        * Receive Data interrupt
...
DEC_BUF:    DS.B    6       * Buffer for number conversion
            EVEN
*----------------------------------------------------------------------
* VEC_OUT - Output character to Console (Let 167-Bug clean up stack)
* Input: D0.B = Character to print
*----------------------------------------------------------------------
    EVEN                    * Force alignment for safety
VEC_OUT
    * Stack-based calling - 167-Bug cleans up the stack for us
    MOVE.B  D0,-(SP)            * Push character parameter
    TRAP    #15
    DC.W    $0020               * .OUTCHR function code
    * NO stack cleanup - 167-Bug does it!
    RTS

*----------------------------------------------------------------------
* VEC_IN - Input character from Console (Minimal version)
* Returns: Carry SET = Char in D0, Carry CLEAR = No Data
*----------------------------------------------------------------------
    EVEN
VEC_IN
    * Check status - no parameters needed
    TRAP    #15
    DC.W    $0001               * .INSTAT function code
    BEQ.S   .NO_DATA            * Z=1 means no data

    * Get Data - per manual, reserve space for result
    SUBQ.L  #2,SP               * Reserve 2 bytes for character (WE allocate)
    TRAP    #15
    DC.W    $0000               * .INCHR function code (167-Bug fills it)
    MOVE.B  (SP),D0             * Get character from stack (don't pop)
    ADDQ.L  #2,SP               * Clean up both bytes at once (stay word-aligned)
    ORI.B   #1,CCR              * Set Carry (Data Ready)
    RTS

.NO_DATA
    ANDI.B  #$FE,CCR            * Clear Carry (No Data)
    RTS
*----------------------------------------------------------------------
* INIT_CD2401 - No-op for TRAP #15 I/O
* 167-Bug handles all hardware, we don't touch anything
*----------------------------------------------------------------------
INIT_CD2401
    RTS
*----------------------------------------------------------------------
* DRAIN_FIFO - Drain receive FIFO
* Clears any leftover characters (e.g., from GO command)
*----------------------------------------------------------------------
DRAIN_FIFO
	MOVEM.L	D0-D1/A0,-(SP)
	LEA	CD2401_BASE,A0
	...
	* Drain up to 20 characters
	MOVEQ	#20,D1
.LOOP
	TST.B	(CyRFOC,A0)
	BEQ.S	.DONE
	MOVE.B	(CyRDR,A0),D0      * Read and discard
	SUBQ.B	#1,D1
	BNE.S	.LOOP
...
* TRAP #15 removed - not supported by 167-Bug

code_start
* Jump directly to BASIC cold start
* (simulator-specific TRAP #15 removed)

* BASIC cold start entry point

LAB_COLD
	* Stack already set at entry point

	* Initialize CD2401 for output
	BSR	INIT_CD2401

	* Initialize vector table
	MOVE.l	#ram_base,a0	* point to RAM base for vector table
   ...
   * This forces the JMP vectors we just wrote to be visible to the CPU.
    DC.W    $F4F8
...
*	MOVE.b	#$F2,Iclim		* default limit for TAB = 14
	...
	BNE.s	LAB_2DAA		* branch if not null (user typed something)
						...
						* character was null so use default ram_top

	MOVE.L	#ram_top,A0		* Set A0 to top of memory ($02000000)
	BRA.s	LAB_2DB6		* Jump to end of detection
...
* --- INSERT THIS FIX HERE ---
    MOVE.L  A0,D1           * Copy the address to D1 to check bits
    BTST    #0,D1           * Check bit 0 (Is it odd?)
    BEQ.S   .IS_EVEN        * If 0, it is even, skip the fix
    ADDQ.L  #1,A0           * If 1, add 1 to make it even
.IS_EVEN
    * --- END OF FIX ---
...
LAB_1295
	...
	* Ensure a0 is aligned to EVEN address before writing line
	MOVE.w	a0,d0			* Copy address to d0
	ANDI.w	#1,d0			* Check if odd (bit 0 set)
	ADD.w		d0,a0			* If odd, add 1 to make even
...
LAB_12E6
	...
	ADDQ.w	#6,a2			* add room for pointer (4) and line # (2)
...
LAB_1301
	...
	MOVE.w	Itemp+2,(a2)+	* save line number (LOWER word for big-endian!)
...
LAB_1359
	JSR		VEC_IN		* call scan input device
...
LAB_SCLN
	...
	CMP.w		(a0),d1		* compare this line # with required line # (word!)
...
LAB_1463
	...
	* Ensure a0 is aligned to EVEN address (robustness check)
	MOVE.l	a0,d0			* Copy address to d0
	BTST	#0,d0			* Check if odd (bit 0 set)
	BEQ.S	.ALIGNED		* If even, skip
	ADDQ.l	#1,a0			* If odd, increment to even
	MOVE.l	a0,Smeml		* Update Smeml to aligned address
.ALIGNED
	MOVEQ		#0,d0			* clear longword
	MOVE.l	d0,(a0)+		* clear first line, next line pointer

	* Clear more memory to avoid garbage (clear 1KB)
	MOVE.W	#255,d1			* loop counter (256 longwords = 1KB)
.clear_loop
	MOVE.L	d0,(a0)+		* clear 4 bytes
	DBF	d1,.clear_loop		* loop

	MOVEA.l	Smeml,a0		* reset to start
...
LAB_14D4
	...
	MOVE.w	(a0)+,d0		* get this line # (word, not longword!)
...
LAB_15DC					* have reached [EOL]+1
	...
	* FIX: Read word into register, clear upper bits, save as LONG
	MOVEQ	#0,d1			* Clear d1
	MOVE.w	(a5)+,d1		* Read line number (16-bit)
	MOVE.l	d1,Clinel		* Store as clean 32-bit longword
LAB_15F6
...
LAB_CLS
    ...
    MOVE.W  #$FF00,d1       * set for clear screen (internal flag)
    MOVE.B  d1,TPos         * clear terminal position internal tracking
    
    * REPLACED TRAP WITH ANSI CALL
    BSR     ANSI_CLS        * Send ANSI Clear Screen codes
    
    RTS
* perform CURSOR X,Y
* perform CURSOR X,Y
LAB_CURSOR
    BSR     LAB_GTBY        * get byte parameter (X), result in d0
    CMP.B   #80,d0          * compare with max+1
    BCC     LAB_FCER        * if >= 80 go do function call error

    MOVE.B  d0,TPos         * set terminal position internal tracker
    MOVE.W  d0,-(sp)        * Save X on stack

    BSR     LAB_SCGB        * scan for "," & get byte param (Y), result in d0

    CMP.B   #32,d0          * compare with max+1 (Row limit)
    BCC     LAB_FCER        * if >= 32 go do function call error

    * Setup call to ANSI_LOCATE
    * D0 already contains Y (Row)
    MOVE.W  (sp)+,d1        * Restore X (Col) into D1
    
    BSR     ANSI_LOCATE     * Move cursor using ANSI codes
    
    * Fix up internal BASIC tracking of cursor position (optional but good)
    * (The original code had logic here for tab tracking, we preserve TPos above)
    RTS
...
LAB_LNCO
	...
	* TRAP #15 (EASy68k disabled) * do i/o function
LAB_LRTS
	RTS
						* do line to X,Y
LAB_L2XY
...
LAB_RUN
	...
	MOVEQ	#0,d0			* Initialize Clinel to 0 (will be set on first line)
	MOVE.l	d0,Clinel		* Clear current line number
...
LAB_18F9
	JSR		VEC_OUT		* output byte via output vector
	CMP.b		#$0D,d0		* compare with [CR]
...
LAB_1880
	JSR		VEC_OUT		* go print the character
...
LAB_19DD
	...
	* FIX: Read word, clear upper bits, save as LONG
	MOVEQ	#0,d1			* Clear d1
	MOVE.w	(a5)+,d1		* Read line number (16-bit)
	MOVE.l	d1,Dlinel		* Store as clean 32-bit longword
LAB_19F6
	BSR		LAB_GBYT		* scan memory
...
LAB_TI
	* Temporary: Return 0 until we implement proper timer
	MOVEQ		#0,d1			* return 0 for now
...
LAB_SMSG
	...
	dc.b	'for the Motorola MVME167 68040 VME SBC',$0D,$0A
...
ram_strt
	...
	ORG	$00500000		* Moved up to avoid code overlap
ram_base
...
cc_throttle	ds.b	1			* Throttle counter (check every N statements)
